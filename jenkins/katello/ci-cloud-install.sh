...
ssh -o StrictHostKeyChecking=no root@$TARGET_HOSTNAME "yum clean all;rm -rf /var/cache/yum*;yum-config-manager --disable \"*\";yum-config-manager --enable \"rhel-6-server-rpms\";yum-config-manager --disable \*cf-tools\*;yum-config-manager --disable \*for-rhel\*;yum-config-manager --disable \*rhev\*;yum repolist"
...
if ! ssh -o StrictHostKeyChecking=no root@$TARGET_HOSTNAME "set -e;yum install ${EXTRA_YUM_OPT[@]} -y $PRODUCT_PACKAGE;katello-configure ${KATELLO_CONFIGURE_OPTS[@]}" ; then
    get_logs 
    exit 1
else 
